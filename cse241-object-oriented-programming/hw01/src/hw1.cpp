...
void Tetromino::rotateCW(){ // transposing and reversing = rotates clockwise 90degree to a vector
    for(int i = 0;i<charvec.size();i++){ // transposing vector
        for(int j =i;j<charvec.size();j++){
            char ctemp = charvec[i][j];
            charvec[i][j] = charvec[j][i];
            charvec[j][i] = ctemp;
        }
    }
    
    for(int i =0;i<charvec.size();i++){ // reversing vector
        int k = charvec.size()-1;
        for(int j = 0;j<charvec.size() / 2;j++){
            char ctemp = charvec[i][j];
            charvec[i][j] = charvec[i][k];
            charvec[i][k] = ctemp;
            k--;
        }
    }
    while(shiftleft()); // checks whether if is there any empty column, if there is;shift vector left
    while(shiftdown()); //  checks whether if is there any empty row, if there is;shift vector down
}
...
void fitGaps(vector<vector<char>> &board){ // removes spaces between letters
    ...
    for(int j=0;j<board[0].size()-1;j++){
        ...
        if(flag == 0){
            ...
            for(int c = j+1;c<board[0].size()-1;c++){
                for(int r = 0;r<4;r++){
